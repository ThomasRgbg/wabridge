# From https://github.com/tgalal/yowsup/blob/master/yowsup/demos/cli/cli.py
# Modfied for non-shell/non-interactive usage (remove readline, threading...)
...
#        self.inputThread.daemon = True
    ...
    def print_usage(self):
        ...
        for cmd, subcommands in self.commands.items():
            for subcmd, subcmdDetails in subcommands.items():
                ...
                # remove the parmeter "user" out again
                out += ("%s " % " ".join(["[%s]" % c for c in subcmdDetails["args"][1:]])).ljust(30)
    ...
    def execCmd(self, cmdInput, user=None):
        ...
        if len(subcmdData["args"]) != len(args):
            print('Number of command arguments wrong: should be {0}, but are {1}'.format(len(subcmdData["args"]),len(args)))
            return self.print_usage()
